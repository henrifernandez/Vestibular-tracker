  ...
  return (
    <div className="space-y-6">
      ...
      {prova.areas.map((area) => {
        const chartData = area.resultados.map((r) => ({
          data: new Date(r.simulado.data).toLocaleDateString("pt-BR"),
          acertos: r.acertos,
        }));
        const metaLinha = area.metas.find((m) => m.acertosAlvo != null)?.acertosAlvo;
          ...
          <section key={area.id} className="bg-white rounded-xl border border-ink/10 p-5">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {area.metas.map((m) => (
                ...
                >
                  {m.label}
                  {m.acertosAlvo != null ? ` · ${m.acertosAlvo}/${area.totalQuestoes} acertos` : ""}
